<template>
  <div class="anclear ">
    ...
    <div class="anclear fff ancontent" >
      <!--按钮组-->
      <!--<ButtonGroup  ref="button1" :button="Flavors.buttons" ></ButtonGroup>-->
      <div class="anclear fff">
        ...
        <div class="anclear fl tableSearch" v-show="Flavors.hasSearch">
          <el-input class="fl" v-model="Flavors.keyword" placeholder="筛选关键词" style="display: inline-block"  ></el-input>
          <el-button class="fr"  type="primary" icon="search" @click="searchFilter(Flavors)" >搜索</el-button>
        </div>
      </div>
      <!--表格-->
      <TablesData class="an-table" ref="multipleTable2" :tableDatas="Flavors" > <!-- :tableDatas 不能改变 table组件就是用的这个接受的参数  -->
        ...
        <el-table-column  prop="mobilePhone" label="临时磁盘"></el-table-column>
        <el-table-column  prop="componey" label="Swap磁盘"></el-table-column>
        <el-table-column  prop="address" label="RX/TX因子" width="200"></el-table-column>
        <el-table-column  prop="phone" label="ID"></el-table-column>
        <el-table-column  prop="phone" label="公有"></el-table-column>
        <el-table-column  prop="phone" label="动作"></el-table-column>

      </TablesData>
      <!--分页-->
      <div class="table-pagination fr">
        <fenYe ref="asdf" :feYeData="Flavors"></fenYe>
      </div>
    </div>
...
//        value4: [new Date(2000, 10, 10, 10, 10), new Date(2000, 10, 11, 10, 10)],//默认
    ...
    methods:{
      ...
      fastSearch(res){
        this.Flavors.keyword=res;
//        console.log(res)
        this.searchFilter(this.Flavors);
      },

      chuangjian(){
        if(this.Flavors.selected.length==1){
      ...
      pop1fun(){
        console.log(this.Flavors.selected);
      },
      shanchu(){
        if(this.Flavors.selected.length==1){
...
//      this.handleSizeChange();
    ...
    mounted:function () {
      this.$nextTick(function () {
        this.searchFilter(this.Flavors);//页面初始化就筛选''也就是全部数据 并且打包分页 与上边一样的功能
        this.Flavors.selected=[];
        this.Flavors.keyword='';
      })
    }
